...
# --- PRE-COMPUTING TOTALS & METRICS (Loop 1) ---
analyzed_portfolio = []
total_cost_basis = 0
...
for position in portfolio:
    # Runs API data fetches ONCE per position and caches them
    analysis = analyse_position(position)
    analyzed_portfolio.append(analysis)
    
    total_cost_basis += analysis['cost_basis']
    total_current_value += analysis['current_value']
    total_gain_loss += analysis['gain_loss']
...
avg_volatility = total_volatility / len(portfolio) if len(portfolio) > 0 else 0

# FIX: Safely initialize portfolio Sharpe variable before accumulating values
portfolio_sharpe_ratio = 0  

# --- GENERATING THE ANALYSIS TABLE & WEIGHTED CALCULATIONS (Loop 2) ---
print()
print("=" * 90)
...
for analysis in analyzed_portfolio:
    ...
    # Calculate relative allocation weight
    weight = (current_value / total_current_value) * 100 if total_current_value > 0 else 0
    
    # FIX: Calculate weighted portfolio Sharpe ratio mathematically
    portfolio_sharpe_ratio += sharpe_ratio * (weight / 100)
    
    print(f"{ticker:<12} {currency}{cost_basis:<11.2f} {currency}{current_value:<14.2f} {currency}{gain_loss:<14.2f} {return_percent:<5.2f}%  {volatility:<5.2f}%  {weight:<5.2f}%")  
    print("=" * 90)
...
print(f"For every 1% of risk, you earn {portfolio_sharpe_ratio:.2f}% of excess return")  
